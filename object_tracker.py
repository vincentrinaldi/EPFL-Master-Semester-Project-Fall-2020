...
def main(_argv):
    ...
    pts_video_blue = np.float32([[277,92],[1920,90],[0,1038],[1920,1038]]) #Blue side 1
    pts_video_mid = np.float32([[0,91],[1920,87],[0,1027],[1920,1029]]) #Mid side 1
    pts_video_white = np.float32([[0,99],[1642,103],[0,1033],[1920,1029]]) #White side 1
    ...
    sngl_frame_state_per_frame = [[None for i in range(3)] for j in range(final_vid_len)] #Vincent
    sngl_nb_detections_per_frame = [[None for i in range(3)] for j in range(final_vid_len)] #Vincent
    tot_rec_points_per_frame = [None for i in range(final_vid_len)] #Vincent
